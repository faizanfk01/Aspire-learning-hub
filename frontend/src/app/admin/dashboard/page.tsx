...
export default function DashboardPage() {
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto">
      {/* Header */}
      <div className="mb-8">
        <p className="text-slate-500 text-sm">{greeting}, <span className="font-semibold text-slate-700">{user?.full_name}</span></p>
        <h1 className="text-2xl font-extrabold text-slate-900 mt-0.5">Dashboard</h1>
      </div>

      {/* Stats */}
      <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-5 gap-4 mb-8">
        <StatCard label="Total Admissions" value={stats?.total_admissions ?? null}
          accent="bg-blue-50" href="/admin/admissions"
        ...
        />
        <StatCard label="Pending Admissions" value={stats?.pending_admissions ?? null}
          accent="bg-amber-50" href="/admin/admissions?status=pending"
        ...
        />
        <StatCard label="Active Students" value={stats?.active_students ?? null}
          accent="bg-green-50" href="/admin/students"
        ...
        />
        <StatCard label="Content Items" value={stats?.total_content ?? null}
          accent="bg-purple-50" href="/admin/content"
        ...
        />
        <StatCard label="Pending Reviews" value={stats?.pending_reviews ?? null}
          accent="bg-orange-50" href="/admin/reviews"
      ...
      <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden">
        ...
        {loading ? (
          ...
          <div className="px-6 py-12 text-center">
            <p className="text-slate-400 text-sm">No pending admissions. You&apos;re all caught up.</p>
          </div>
        ) : (
          <table className="w-full">
            <thead>
              <tr className="bg-slate-50 border-b border-slate-100">
                {["Student", "Parent / Guardian", "Grade", "Account Email", "Applied", ""].map((h) => (
            ...
            <tbody className="divide-y divide-slate-100">
              {recent.map((a) => (
                <tr key={a.id} className="hover:bg-slate-50 transition-colors">
                  ...
                  <td className="px-5 py-3.5 text-sm text-slate-600 whitespace-nowrap">{a.father_name}</td>
                  <td className="px-5 py-3.5 text-sm text-slate-600 whitespace-nowrap">{a.grade}</td>
